...
import { ScreenShareView } from "../new/components/ScreenShareView";
...
function getVisibleParticipants(
  participants: (ChannelHuddleParticipant | null)[],
  getParticipantState: (participant: ChannelHuddleParticipant) => {
    isMuted: boolean;
    isSpeaking: boolean;
    isWaiting: boolean;
  },
  ...
  visible: ChannelHuddleParticipant[];
  remaining: number;
} {
  const validParticipants = participants.filter(
    (p): p is ChannelHuddleParticipant => p !== null && p.user !== null
  );
    ...
    >
      ...
      >
        {visible.map((participant, index) => {
          ...
          // We've already checked participant.user exists above
          return (
            <ParticipantCard
              key={participant._id}
...
function ParticipantCard({
  ...
  participant: ChannelHuddleParticipant;
  isYou: boolean;
  isWaiting: boolean;
...
export function ChannelHuddleDialog({
  ...
  const [showChannelHuddleDialog, setShowChannelHuddleDialog] =
    useShowChannelHuddleDialog();
  ...
  return (
    <Dialog
      open={showChannelHuddleDialog}
      onOpenChange={setShowChannelHuddleDialog}
    >
      <DialogContent
        className={`${
          ...
          {hasScreenShare ? (
            <ScreenShareView
              ...
              getParticipantState={(participant) =>
                getParticipantState(participant as ChannelHuddleParticipant)
              }
